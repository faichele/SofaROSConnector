...
class ROSMessageBindingGenerator(ROSBindingGenerator):
    ...
    def __init__(self, generate_factory_method, base_directory, ros_message_types = []):
        ROSBindingGenerator.__init__(self)
        self.__generate_factory_method = generate_factory_method
...
ZyROSPublisher& ZyROSPublisher::operator=(const ZyROSPublisher& other)
{
    ...
    def generate_ros_message_subscriber_files(self):
        ...
        if len(header_files_per_message_type.keys()) > 0:
            ...
            self.logger.info('Writing header file for messages to: ' + message_source_file_path)

            message_cpp_file_path = os.path.join(self.__base_directory, self.__ros_message_subscribers_generated_cpp_name)
            self.logger.info('Writing CPP file for messages to   : ' + message_cpp_file_path)
            ...
            for message_type in sorted(header_files_per_message_type.keys()):
                ...
                if not message_type_is_excluded:
                    for header_file in header_files_per_message_type[message_type]:
                        header_file_path = os.path.join(self.ros_include_dir, header_file)
                        if os.path.exists(header_file_path):
                            self.logger.debug('Including message header ' + header_file + ': ' + header_file_path + ' exists.')
                            message_source_file.write('#include <' + header_file + '>\n')
                        else:
                            self.logger.debug('Not including ' + header_file + ': ' + header_file_path + ' does not exist!')

            message_source_file.write('\n\n')

            message_source_file.write('#include <boost/shared_ptr.hpp>\n')

            if self.__generate_factory_method:
    ...
    def generate_ros_message_publisher_files(self):
        ...
        if len(header_files_per_message_type.keys()) > 0:
            ...
            self.logger.info('Writing message publisher header file to : ' + message_source_file_path)

            message_cpp_file_path = os.path.join(self.__base_directory, self.__ros_message_publishers_generated_cpp_name)
            self.logger.info('Writing message publisher CPP file for to: ' + message_cpp_file_path)
            ...
            message_source_file.write(self.__message_publisher_cpp_file_static + '\n\n')
            ...
            if self.__generate_factory_method:
                ...
                self.logger.info('SUPPOSEDLY not writing message publisher auto-generated factory method code!')

            message_cpp_file.close()

    def generate_binding_sources(self):
        self.logger.info('Generating ROS message binding sources. Message definitions passed: ' + str(len(self.__ros_message_types)))
        self.logger.info('Factory methods generation set to: ' + str(self.__generate_factory_method))
...
class ROSServiceBindingGenerator(ROSBindingGenerator):
    ...
    def __init__(self, generate_factory_method, base_directory, ros_service_types = []):
        ROSBindingGenerator.__init__(self)
        self.__generate_factory_method = generate_factory_method
...
void ZyROSConnectorServiceServer::serverLoop()
{
    ...
    if (m_d->m_rosNodeHandle)
    {
        ...
        self.__excluded_service_names = ['spencer_mapping_msgs/Label'] # These break compilation when included directly under ROS kinetic/melodic
    ...
    def generate_binding_sources(self):
        ...
        if len(header_files_per_service_type.keys()) > 0:
            ...
            service_cpp_file_path = os.path.join(self.__base_directory, self.__ros_service_client_generated_cpp_name)
            self.logger.info('Writing CPP file for services to: ' + service_cpp_file_path)

            service_server_cpp_file_path = os.path.join(self.__base_directory, self.__ros_service_server_generated_cpp_name)
            ...
            for service_type in sorted(header_files_per_service_type.keys()):
                ...
                if not service_is_excluded:
                    for header_file in header_files_per_service_type[service_type]:
                        header_file_path = os.path.join(self.ros_include_dir, header_file)
                        if os.path.exists(header_file_path):
                            self.logger.debug('Including service header ' + header_file + ': ' + header_file_path + ' exists.')
            ...
            service_cpp_file = open(service_server_cpp_file_path,  'w')
...
if __name__ == '__main__':
    ...
    logging.info("===========================================================")
    logging.info("ROS message and service type file generation starting HERE.")
    logging.info("===========================================================")

    parser = argparse.ArgumentParser()
    parser.add_argument("--generate_factory_methods", help="Generate factory methods for topic subscribers/publishers and service clients", type=str, default="False")
    ...
    logging.info("Arguments: generate_factory_methods=" + str(args.generate_factory_methods) + ", base_directory=" + str(args.base_directory))

    generate_factory_methods = False
    if str(args.generate_factory_methods) == 'True':
        generate_factory_methods = True
    elif str(args.generate_factory_methods) == 'False':
        generate_factory_methods = False
    else:
        logger_main.error('generate_factory_methods arguments has to be either string \'True\' or string \'False\'! Value given was: ' + str(args.generate_factory_methods))
        sys.exit(1)
    ...
    if args.message_types != '':
        ...
        message_binding_generator = ROSMessageBindingGenerator(generate_factory_method=generate_factory_methods, base_directory=args.base_directory, ros_message_types=message_types_list)
    ...
    if args.service_types != '':
        ...
        service_binding_generator = ROSServiceBindingGenerator(generate_factory_method=generate_factory_methods, base_directory=args.base_directory, ros_service_types=service_types_list)
